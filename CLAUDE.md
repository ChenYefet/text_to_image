...
Derivational prefixes — bound morphemes that attach to a head noun via a hyphen to form a prefixed compound, such as 'sub-' in sub-agent or sub-task, 'pre-' in pre-condition, 'post-' in post-processing, 'co-' in co-author, 'non-' in non-member, 'multi-' in multi-tenant, or any other derivational prefix — are not abbreviations and are not subject to the abbreviation registry. They combine freely with head nouns, including recursively (e.g. sub-sub-agent), without requiring a registry entry. The resulting prefixed compound inherits the status of its head noun: if the head noun is itself an established technical compound term, the prefixed compound is treated as a prefixed instance of that compound; otherwise, the prefixed compound is a plain noun bearing a prefix.
