...
When a colon introduces post-colon content, do not capitalise the first word of that content, except where that word is a proper noun, the pronoun 'I', or the start of a quotation that originally began with a capital. This default applies uniformly across inline prose, bulleted lists, and numbered lists, and is unaffected by whether the post-colon content is an independent clause, a phrase, a subordinate clause, or a list, and unaffected by whether the introductory clause contains a forward-pointing reference (such as 'the following X', a demonstrative, or a label-style introducer that announces content to follow). Items in displayed lists (bulleted or numbered) follow the same default: each item begins with a lowercase letter unless its first word is a proper noun, the pronoun 'I', or the start of a quotation that originally began with a capital. Avoid the construction '[noun phrase] is: [full clause]'; instead restructure to make the clause the direct complement of its governing verb, or lead directly with the normative subject (e.g. not 'The normative behaviour for X is: the service shall return…' but 'For X, the service shall return…').
...
No abbreviations or acronyms are permitted in code identifiers (variable names, function names, class names, import aliases, filenames) or in prose (inline comments, docstrings, commit messages, documentation, section headings, diagram labels, table cells, configuration examples, and user-facing output strings) unless they appear in the abbreviation registry at the end of this document. Each entry in that registry records two independent decisions: whether the abbreviation remains abbreviated in code identifiers, and whether it remains abbreviated in prose. An entry governs only the specific abbreviation/expansion pair it records — the same letters used as an abbreviation of a different term constitute a distinct pair that the existing entry does not govern. For example, the registry entry "sub | subsystem" governs uses of "sub" as an abbreviation of "subsystem"; it does not govern a use of "sub" as an abbreviation of "subordinate", "substitute", or any other term, and it imposes no obligation on those other uses by virtue of the shared letters alone. When you encounter a use of an abbreviation or acronym for which the registry has no governing entry — whether because the letters do not appear in the registry at all, or because no existing entry for those letters covers the current expansion — prompt the user to decide, for each context independently (code identifiers and prose), whether the abbreviation should be retained in abbreviated form or replaced with its expansion, then record both decisions as a new row. When an entry's marking for a given context (code identifiers or prose) is "expanded", the expanded form shall always be used in that context whenever the abbreviation is being used for the expansion the row records. Conversely, when an entry's marking for a given context is "abbreviated", the abbreviated form shall always be used in that context whenever the abbreviation is being used for the expansion the row records. The following categories of symbols must never be modified regardless of whether they contain abbreviations:
...
- 'alongside' — spatial accompaniment or adjacency to another item, with the two items remaining distinct rather than one containing the other (notes_alongside_deliverable, citations_alongside_plan); distinct from 'with', which denotes pairing or accompaniment of any kind without the spatial-adjacency sense, and from 'next to', which is not a single-word preposition
- 'inside' — interior spatial containment with explicit reference to the interior region of the container (action_inside_transaction, sub_agent_running_inside_parent_context); distinct from 'in', which denotes general structural containment without the spatial-interior emphasis
- 'outside' — exterior to a bounded region, the spatial inverse of 'inside' (caching_outside_repository, fetch_outside_audit_envelope); also used for scopes that lie beyond a defined boundary
- 'within' — bounded interior containment with the boundary itself in focus, often used for ranges, windows, or invocation scopes (delivery_within_window, growth_within_capacity, transition_within_invocation); distinct from 'in' (general containment without boundary emphasis) and from 'inside' (which emphasises the spatial interior over the boundary that defines it)
- 'instead of' / 'in lieu of' — substitution, the use of one item in place of another (cached_body_in_lieu_of_re_fetch, sub_agent_dispatch_instead_of_inline_channelling); the two forms are synonymous and interchangeable, with 'in lieu of' carrying a more formal register; no single-word preposition in the list above captures substitution, which is why this phrasal entry is admitted
...
A separate failure mode from the wholly mechanical subject covered above is the bundled subject that names the intent correctly but then trails one or more mechanism clauses on after it — for example, a subject of the form "Close the ambiguity whereby X — by adding helper Y, by restructuring file Z, by replacing P with Q — bringing the module into parity with the sibling module — closing the gap whereby Z". Even though every layer of this subject is intent-grounded in the sense that the verb names a deficiency, the three 'by ...' clauses, the 'bringing into parity' clause, and the 'closing the gap whereby' clause are mechanism: each names a specific edit, helper, file, file path, artefact, or comparison that the diff already reveals. The compliant form is a subject of "Close the ambiguity whereby X" — with X elaborated up the intent chain to the governing principle — paired with a body of bullets, one per decision, each describing the decision and its rationale: one bullet for adding helper Y, one for the restructuring of file Z, one for the substitution of Q for P, and one for the parity claim.

Apply this scan to every subject line before finalising the message, identifying any clause introduced by 'by [verb-ing]', 'closing the gap whereby [mechanism description]', 'bringing X into parity with Y', 'replacing X with Y', or any other construction that names a specific edit, helper, file, file path, or artefact name that the diff would already reveal. Each such clause is mechanism. Mechanism shall be moved to a body bullet — never retained in the subject merely because it can grammatically follow the intent description. The subject must contain only the layers of the ascending intent chain (the deficiency, its manifestation, its consequence, the foundational property that makes the consequence bad, and the governing principle); the body's bullets must record the decisions taken with their rationale, not summarise the diff. A subject that ascends through every intent layer and then trails 'by [mechanism]' clauses still violates this directive — the intent layers do not absolve the mechanism clauses, and the mechanism clauses must be extracted to the body even when the subject's intent ascent is otherwise complete.
...
Consider finally a change to the abbreviation registry, whose purpose is reached at the third layer. Starting form: "Close the gap in CLAUDE.md's abbreviation registry where DTO had no prescribed handling". Successive intent layers:

1. "Extend CLAUDE.md's abbreviation registry with DTO" — adds the registry's scope ("abbreviation"); the reader asks 'but why add DTO?'.
...
The three cases reach the purpose at different depths because the chain from artefact to domain-baseline knowledge has different length in each: in the registry case the principle is close; in the test case a consequence intercedes; in the prohibition case both a consequence and a foundational property intercede before the principle. The number of layers is incidental — the rule is to keep ascending while the next 'but why?' would require project-specific reasoning to answer.
...
4. an industry standard or established convention — a value that is widely adopted in the relevant engineering domain and documented in a verifiable reference. The citation must include all three of the following: the name of the standard, convention, or framework that establishes the value; the domain in which the value applies (the system, layer, or class of components the convention governs); and a verifiable locator. A verifiable locator is at least one of a URL to the documentation, a document title with section, chapter, or page number, a source-code or configuration file path with line number, a Request for Comments number or other standards body identifier, or a framework command-line flag together with the framework name and version — that is, an identifier sufficient for the reader to retrieve the source without further investigation (e.g. "Kubernetes default `--node-status-update-frequency` of 10 seconds, kubelet command-line reference, https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet/, Kubernetes 1.29"; "control system hysteresis ratio of 0.4–0.6 for threshold-based switching, Åström and Murray, *Feedback Systems: An Introduction for Scientists and Engineers*, Princeton University Press, 2008, section 11.3"). If a verifiable locator cannot be produced, the value does not satisfy this source: it must be re-grounded in sources 1, 2, or 3, or re-classified as source 5 (first-principles reasoning) and justified from the underlying physical, mathematical, or engineering constraints — never as an unsupported assertion of common practice.
...
For every numerical value, the answer to "why not half, why not double" must be made explicit, but the form of the answer depends on the grounding source. For values grounded in source 1 (specification or document reference), source 3 (published benchmark), or source 4 (industry standard or established convention), the source itself is the answer: the specification prescribes the value, the benchmark measures it, or the convention adopts it. The required statement is the citation already produced under that source — no further speculative analysis is required, and producing speculative half/double reasoning when a source pins the value is a violation of this rule. For values grounded in source 2 (mathematical derivation from documented parameters), the answer is determined by the inputs and the formula: a different value would imply different inputs or a different formula, both of which must themselves be grounded in one of the listed sources. State this explicitly rather than treating the derived value as independently calibrated. For values grounded in source 5 (first-principles reasoning), the half/double analysis is the primary sensitivity check on the calibration choice. State why the value is not half of what is prescribed and why it is not double; if halving or doubling produces no material change in the design's behaviour, state this — the value may be less important than it appears; if halving or doubling produces a failure mode, name the failure mode. The half/double obligation does not apply to: numbers determined by complete enumeration (where the count is the result of listing all items and no design latitude exists), literal references (such as line numbers, section numbers, or HTTP status codes), and mathematical constants.
