...
When bumping the version of the specification document, the specification file shall be renamed to reflect the new version number, following the established pattern `text_to_image_specification_version_{major}_{minor}_{patch}.md`. The rename shall be included in the same commit that updates the document version and changelog.
