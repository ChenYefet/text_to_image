...
When a colon introduces a complete independent clause — one that could stand as a sentence in its own right — capitalise the first word of that clause, unless the colon is functioning as a comma substitute within a single sentence rather than as a sentence boundary. To diagnose the comma-substitute case, check whether replacing the colon (and any list-item delimiters such as bullets or numbering, if the post-colon content is rendered as a list) with commas yields a single grammatically valid sentence rather than a comma splice; this diagnostic applies regardless of whether the post-colon content is rendered as inline prose, as a bulleted list, or as a numbered list. In 'For each risk: state the risk, cite where the gap appears, and explain the failure mode' — and equally in the same content rendered as bullet points beginning with 'state', 'cite', and 'explain', or as a numbered list with the same items — the comma form 'For each risk, state the risk, cite where the gap appears, and explain the failure mode' is grammatically valid (the prepositional phrase 'For each risk' modifies the imperative that follows), so the colon is a comma substitute and the post-colon content is not capitalised in any of these renderings. In 'Result: The system processed all transactions', the comma form 'Result, The system processed all transactions' would be a comma splice, so the colon is a sentence boundary and 'The' is capitalised. The comma-substitute diagnostic is overridden when the introductory clause contains a cataphoric noun phrase — a noun or noun phrase that the colon anticipates and the post-colon items fulfil. Cataphoric noun phrases take several forms: 'the following X' ('the following categories of symbols', 'the following grounding sources'), demonstratives that point forward ('this procedure', 'these requirements'), label-style introducers that announce content to follow ('Worked example — ...', 'Definition', 'Note', 'The diagnostic test', 'The test'), and quantified noun phrases that the post-colon items fulfil ('both perspectives', 'three independent validations', 'one exception'). When such a cataphoric noun phrase is present, the post-colon items take initial capitalisation regardless of whether the comma form would otherwise constitute a single grammatically valid sentence, because each item is a self-standing fulfilment of the cataphoric reference rather than a continuation of the syntactic flow of the introductory clause. When the introductory clause contains no cataphoric noun phrase — when the post-colon items are direct objects of a verb in the introductory clause, or when the introductory clause is a temporal or conditional scope-setter without a cataphoric noun — the comma-substitute diagnostic governs as stated above. When the colon introduces a phrase, subordinate clause, or list that is grammatically dependent on what precedes it, do not capitalise. Avoid the construction '[noun phrase] is: [full clause]'; instead restructure to make the clause the direct complement of its governing verb, or lead directly with the normative subject (e.g. not 'The normative behaviour for X is: the service shall return…' but 'For X, the service shall return…').
