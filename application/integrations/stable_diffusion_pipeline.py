"""
In-process image generation pipeline (``StableDiffusionPipeline``) backed by
the HuggingFace ``diffusers`` library.

This pipeline loads a Stable Diffusion model directly in-process using the
HuggingFace ``diffusers`` library.  The model is downloaded automatically
from HuggingFace Hub on first run and cached locally for subsequent starts.

Memory management
-----------------
After each inference call (whether successful or failed), the pipeline
...
``diffusers`` package) alongside generated images.  This pipeline
...
real user request, the pipeline offers a ``run_startup_warmup`` method
...
If the warmup is not performed (or fails), the pipeline falls back to the
...
class StableDiffusionPipeline:
    """
    In-process image generation pipeline backed by the ``diffusers``
    ``StableDiffusionPipeline`` from HuggingFace.
    ...
    def load_pipeline(
        ...
        Args:
            ...
            thread_pool_executor_for_inference: A ``ThreadPoolExecutor``
                ...
                through to the ``StableDiffusionPipeline`` constructor.
    ...
    async def run_startup_warmup(self) -> None:
        ...
        try:
            ...
            # pipeline continues normally and the first user request will
    ...
    async def generate_images(
        ...
        # v5.10.0 specification (Section 18, event B-4).
...
class ImageGenerationResult:
    ...
    pipeline (which works with PIL images and PyTorch tensors) and the
