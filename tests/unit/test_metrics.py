...
- Returns ISO 8601 UTC timestamps for ``collected_at`` and
  ``service_started_at`` as required by the v5.8.0 specification (FR38).
...
# microsecond precision and the 'Z' suffix, as required by the v5.8.0
...
class TestMetricsCollectorLatencyStatistics:
    ...
    def test_latency_aggregates_across_status_codes(self):
        ...
        the same method and path combination, as prescribed by the v5.8.0
        specification."""
        collector = application.metrics.MetricsCollector()
...
class TestMetricsCollectorBoundedRetention:
    ...
    def test_default_maximum_number_of_observations_matches_specification(self):
        ...
        the v5.8.0 specification's expected evaluation scope."""
